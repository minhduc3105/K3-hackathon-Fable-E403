...
function QuizConfigModal({ state, onClose, onCreate, onPatch }: { state: DemoState; onClose: () => void; onCreate: () => void; onPatch: (value: Partial<DemoState>) => void }) {
  ...
  return <><div className="modal-backdrop" onClick={onClose} /><div className="quiz-config-modal" role="dialog" aria-labelledby="modal-title"><button className="modal-close" type="button" onClick={onClose} aria-label="Đóng"><X size={20} /></button><div className="modal-header"><Sparkle size={24} /><h2 id="modal-title">Thẻ thông tin</h2></div><div className="modal-body"><section className="config-section"><h3>Số lượng thẻ</h3><div className="config-toggle"><button className={quantity === "few" ? "active" : ""} type="button" onClick={() => applyQuantity("few")}>Ít hơn</button><button className={quantity === "standard" ? "active" : ""} type="button" onClick={() => applyQuantity("standard")}>Tiêu chuẩn (Mặc định)</button><button className={quantity === "custom" ? "active" : ""} type="button" onClick={() => applyQuantity("custom")}>Tùy chọn khác</button></div>{quantity === "custom" && <input type="number" min="1" max="20" value={state.quizQuestionCount} onChange={(e) => onPatch({ quizQuestionCount: Math.max(1, Math.min(20, Number(e.target.value) || 1)) })} />}</section><section className="config-section"><h3>Nguồn</h3><select value="current" disabled><option value="current">1 nguồn</option></select></section><section className="config-section"><h3>Chủ đề nên là gì?</h3><textarea value={state.learnerIntent} maxLength={500} onChange={(e) => onPatch({ learnerIntent: e.target.value })} placeholder="Đặt công thức câu trúc JTBD lên mặt trước và ví dụ áp dụng lên mặt sau." rows={4} /><div className="modal-hint"><span>Tab →</span></div></section></div><button className="primary-button modal-create" type="button" onClick={onCreate} disabled={state.screen === "processing"}>Tạo</button></div></>;
}
