...
export function renderQuiz(state) {
  ...
  const choices = question.choices.map((choice, index) => `<label class="choice ${selectedAnswer === choice.id ? "is-selected" : ""}"><input type="radio" name="answer" value="${choice.id}" ${selectedAnswer === choice.id ? "checked" : ""} data-action="select-answer" /><span class="choice-key">${String.fromCharCode(65 + index)}</span><span class="choice-label">${escapeHtml(choice.label)}</span></label>`).join("");
